...
return new class extends Migration {
    ...
    {
        Schema::create('ad_cubusiness_has_return_product_item', function (Blueprint $table) {
            $table->id();
            ...
            $table->double('return_quantity', 22, 2);
            ...
            $table->foreign('ad_cubusiness_has_invoice_id', 'fk_ret_prod_item_invoice_id')
                ->references('id')
                ->on('ad_cubusiness_has_invoice')
                ->onDelete('restrict');
            $table->foreign('pm_product_item_id', 'fk_ret_prod_item_id')
                ->references('id')
                ->on('pm_product_item')
                ->onDelete('restrict');
            $table->foreign('stm_branch_stock_id', 'fk_ret_prod_branch_stock_id')
                ->references('id')
                ->on('stm_branch_stock')
                ->onDelete('restrict');
        });
    ...
    {
        Schema::dropIfExists('ad_cubusiness_has_return_product_item');
    }
};
