...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <!-- Agent Picker -->
                <div class="relative">
                    <select id="summary-agent"
                        class="form-select pl-9 text-slate-500 hover:text-slate-600 font-medium focus:border-slate-300 w-full md:w-60 bg-white border-slate-200 rounded-lg shadow-sm px-3 py-2 appearance-none">
                    ...
                    <div class="absolute inset-0 right-auto flex items-center pointer-events-none">
                        <i class="bi bi-person-badge text-slate-500 ml-3"></i>
                    </div>
                </div>

                <!-- Start Date Picker -->
                <div class="relative">
                    <input id="summary-start-date"
                        class="form-input pl-9 text-slate-500 hover:text-slate-600 font-medium focus:border-slate-300 w-full md:w-44 bg-white border-slate-200 rounded-lg shadow-sm px-3 py-2"
                        type="date" />
                    <div class="absolute inset-0 right-auto flex items-center pointer-events-none">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white shadow-sm rounded-xl border border-slate-200 p-6">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <button id="view-returns-details-btn" class="text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-600 font-semibold px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 shadow-sm">
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden transform transition-all">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <i class="bi bi-arrow-return-left text-indigo-600"></i> Customer Returns Details
                </h3>
                <button id="close-returns-modal"
                    class="text-slate-400 hover:text-slate-600 transition-colors p-1 rounded-md hover:bg-slate-100">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            ...
            <div class="p-6">
                <div class="overflow-y-auto max-h-[60vh] pr-1 space-y-4" id="returns-modal-content">
                    <!-- Populated by JS -->
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const closeReturnsModalBtn = document.getElementById('close-returns-modal');
            const viewReturnsDetailsBtn = document.getElementById('view-returns-details-btn');
            const returnsModalContent = document.getElementById('returns-modal-content');
            ...
            const fetchData = async () => {
                ...
                try {
                    let url = `/api/web-daily-summary?start_date=${startDate}&end_date=${endDate}&agent_id=${agentId}`;
                    ...
                    if (result.status && result.data) {
                        ...
                        currentReturnsData = data.returns.customer_returns || [];
                        ...
                        if (loadHeader) {
                            if (startDate === endDate) {
                                loadHeader.innerText = `Daily Loads on ${startDate}`;
                            } else {
                                loadHeader.innerText = `Daily Loads from ${startDate} to ${endDate}`;
                            }
                        }
            ...
            const openLoadModal = async (loadId) => {
                ...
                try {
                    const response = await fetch(`/api/web-daily-summary?start_date=${startDate}&end_date=${endDate}&load_id=${loadId}`);
                    const result = await response.json();
            ...
            // Returns Modal Logic
            viewReturnsDetailsBtn.addEventListener('click', () => {
                returnsModal.classList.remove('hidden');
                returnsModalContent.innerHTML = '';

                if (currentReturnsData.length === 0) {
                    returnsModalContent.innerHTML = '<div class="text-center py-12 text-slate-400 font-medium">No return records found for this date range.</div>';
                    return;
                }

                currentReturnsData.forEach((customer, idx) => {
                    ...
                    let rowsHtml = customer.items.map(item => `
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="py-2.5 px-3 text-right font-bold text-rose-600">${formatCurrency(item.total_price)}</td>
                            <td class="py-2.5 px-3 pl-4 text-slate-500 text-xs italic">${item.reason}</td>
                        </tr>
                    `).join('');

                    card.innerHTML = `
                        <div class="p-4 bg-slate-50/70 flex justify-between items-center border-b border-slate-200">
                            <span class="font-bold text-slate-800 text-sm flex items-center gap-2">
                                <i class="bi bi-shop text-indigo-500"></i> ${customer.business_name}
                            </span>
                            <span class="font-bold text-rose-600 text-sm">Total: ${formatCurrency(customer.total_value)}</span>
                        </div>
                            ...
                            <table class="table-auto w-full text-xs text-left">
                                <thead>
                                    <tr class="text-slate-500 uppercase border-b border-slate-200 font-bold">
                                        ...
                                        <th class="pb-2 px-3 text-right">Total Price</th>
                                        <th class="pb-2 px-3 pl-4">Reason</th>
                                    </tr>
                                </thead>
                    ...
                    `;
                    returnsModalContent.appendChild(card);
                });
            });

            closeReturnsModalBtn.addEventListener('click', () => {
