@php
    ...
    $approval_workflow = $settings->approval_workflow ?? (object)[];
    $notifications = $settings->notifications ?? (object)[];
@endphp
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        <div class="p-6">
            ...
            <div class="space-y-3">
                ...
                <div id="task-card-{{ $task->id }}" class="border rounded-lg p-4 transition-colors duration-200 {{ ($task->required ?? false) ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200' }}">
                    <div class="flex items-start gap-4">
                        {{-- Order Number --}}
                        <div class="flex-shrink-0">
                            <div id="task-badge-{{ $task->id }}" class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold {{ ($task->required ?? false) ? 'bg-red-500 text-white' : 'bg-gray-400 text-white' }}">
                                {{ $task->order }}
                            </div>
                        </div>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <input type="checkbox" name="tasks[{{ $task->id }}][required]" id="required_{{ $task->id }}" value="1"
                                        {{ old("tasks.{$task->id}.required", $task->required ?? false) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-center gap-2 h-10">
                    <input type="checkbox" name="approval_workflow[require_approval]" id="require_approval" value="1"
                        {{ old('approval_workflow.require_approval', $approval_workflow->require_approval ?? false) ? 'checked' : '' }}
                ...
                <div id="approval-options" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 transition-all duration-300 {{ old('approval_workflow.require_approval', $approval_workflow->require_approval ?? false) ? '' : 'hidden opacity-50' }}">
                    <div>
                        ...
                        <select name="approval_workflow[approver_role]" id="approver_role" 
                            ...
                            @php $role = $approval_workflow->approver_role ?? 'store_manager'; @endphp
                    ...
                    <div>
                        ...
                        <select name="approval_workflow[auto_lock_timing]" id="auto_lock_timing" 
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#D4A017] focus:ring-[#D4A017] sm:text-sm p-2">
                            @php $timing = $approval_workflow->auto_lock_timing ?? 'immediate'; @endphp
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="notifications[reminder_time]" id="reminder_time" 
                        value="{{ old('notifications.reminder_time', $notifications->reminder_time ?? '') }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#D4A017] focus:ring-[#D4A017] sm:text-sm p-2">
                ...
                <div>
                    ...
                    <input type="time" name="notifications[escalate_time]" id="escalate_time" 
                        value="{{ old('notifications.escalate_time', $notifications->escalate_time ?? '') }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#D4A017] focus:ring-[#D4A017] sm:text-sm p-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if(isset($notifications->recipients))
                            @foreach((array)$notifications->recipients as $email)
