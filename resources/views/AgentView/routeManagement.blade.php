...
@section('content')
    <div class="min-h-screen bg-[#EDEFF5]">
        ...
        <div class="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 sm:py-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div class="flex-shrink-0 flex gap-2">
                    <button onclick="openRouteModal()" class="bg-[#D4A017] hover:bg-[#B8860B] text-white px-4 py-2.5 rounded-xl flex items-center gap-2 shadow-sm text-sm font-semibold transition-all transform active:scale-95 cursor-pointer">
                ...
                <div id="mapInfoBar" class="hidden bg-white/95 backdrop-blur-sm border-b border-gray-200 px-5 py-3 z-10">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <button id="editRouteBtn" onclick="editRoute(selectedRouteId)" class="bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm text-xs font-bold transition-all transform active:scale-95 cursor-pointer">
                                <i class="bi bi-pencil-fill"></i>
                                Edit Details
                            </button>
                            <a id="routeBuilderBtn" href="#" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm text-xs font-bold transition-all transform active:scale-95 cursor-pointer">
        ...
        <div id="route-modal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="route-modal-title" role="dialog" aria-modal="true">
            <div id="route-backdrop" class="fixed inset-0 bg-gray-900/70 transition-opacity opacity-0 duration-300 ease-out" onclick="closeRouteModal()"></div>
            <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
                <div id="route-panel" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100 opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95 transition-all duration-300 ease-out flex flex-col">
                    <!-- Modal Header -->
                    <div class="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-4 flex items-center justify-between text-white flex-shrink-0">
                        <div class="flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
                                <i class="bi bi-sign-turn-right text-lg"></i>
                            </div>
                            <div>
                                <h3 class="font-bold text-base" id="routeModalTitle">Create Route</h3>
                                <span class="text-[10px] text-amber-100 tracking-wider uppercase font-semibold" id="routeModalSubtitle">Add details for a new route</span>
                            </div>
                        </div>
                        <button onclick="closeRouteModal()" class="text-white/80 hover:text-white transition-colors">
                            <i class="bi bi-x-lg text-lg"></i>
                        </button>
                    </div>
                    ...
                    <form id="routeForm" onsubmit="saveRoute(event)">
                        @csrf
                        <input type="hidden" id="routeIdField" name="id" value="">
                        
                        <div class="px-6 py-5 space-y-4 overflow-y-auto max-h-[60vh]">
                            <!-- Route Name -->
                            <div>
                                <label for="routeNameInput" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Route Name *</label>
                                <input type="text" id="routeNameInput" name="route_name" required placeholder="e.g. Colombo Outer Circular Route"
                            ...
                            <!-- Description -->
                            <div>
                                <label for="routeDescriptionInput" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Description</label>
                                <textarea id="routeDescriptionInput" name="description" rows="3" placeholder="Describe the coverage area or specifics of the route..."
                            ...
                            <!-- Target Metrics (Distance & Duration) -->
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label for="routeDistanceInput" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Target Distance (km)</label>
                                    <input type="number" step="0.01" id="routeDistanceInput" name="target_distance_km" placeholder="e.g. 45.5"
                                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400 text-sm bg-white font-medium shadow-sm transition-all text-gray-700">
                                </div>
                                <div>
                                    <label for="routeDurationInput" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Target Duration (hours)</label>
                                    <input type="number" step="0.1" id="routeDurationInput" name="target_duration_hours" placeholder="e.g. 4.5"
                                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400 text-sm bg-white font-medium shadow-sm transition-all text-gray-700">
                                </div>
                            </div>
                            ...
                            <!-- Status -->
                            <div>
                                <label for="routeStatusInput" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Status</label>
                                <select id="routeStatusInput" name="status"
                                    class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400 text-sm bg-white font-medium shadow-sm transition-all text-gray-700">
                        ...
                        <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-100 flex-shrink-0">
                            ...
                            <button type="submit" id="saveRouteBtn"
    ...
    <script>
        ...
        const routeBackdrop = document.getElementById('route-backdrop');
        const routePanel = document.getElementById('route-panel');
        const routeForm = document.getElementById('routeForm');

        function openRouteModal() {
            routeForm.reset();
            document.getElementById('routeIdField').value = '';
            document.getElementById('routeModalTitle').innerText = 'Create Route';
            document.getElementById('routeModalSubtitle').innerText = 'Add details for a new route';
            
            routeModal.classList.remove('hidden');
            void routeModal.offsetWidth;
            routeBackdrop.classList.remove('opacity-0');
            routeBackdrop.classList.add('opacity-100');
            routePanel.classList.remove('opacity-0', 'translate-y-4', 'sm:translate-y-0', 'sm:scale-95');
            routePanel.classList.add('opacity-100', 'translate-y-0', 'sm:scale-100');
        }

        function closeRouteModal() {
            routeBackdrop.classList.remove('opacity-100');
            routeBackdrop.classList.add('opacity-0');
            routePanel.classList.remove('opacity-100', 'translate-y-0', 'sm:scale-100');
            routePanel.classList.add('opacity-0', 'translate-y-4', 'sm:translate-y-0', 'sm:scale-95');
            setTimeout(() => { routeModal.classList.add('hidden'); }, 300);
        }
        ...
        function editRoute(routeId) {
            ...
            document.getElementById('routeIdField').value = route.id;
            document.getElementById('routeNameInput').value = route.routeName || '';
            document.getElementById('routeDescriptionInput').value = route.description || '';
            document.getElementById('routeDistanceInput').value = route.target_distance_km || '';
            document.getElementById('routeDurationInput').value = route.target_duration_hours || '';
            document.getElementById('routeStatusInput').value = route.status || '1';

            document.getElementById('routeModalTitle').innerText = 'Edit Route';
            document.getElementById('routeModalSubtitle').innerText = 'Modify details for this route';
        ...
        function saveRoute(event) {
            ...
            const routeId = document.getElementById('routeIdField').value;
            const url = routeId ? `/api/routes/${routeId}/update` : '/api/routes/create';
            const formData = new FormData(routeForm);

            const data = {
                route_name: formData.get('route_name'),
                description: formData.get('description'),
                target_distance_km: formData.get('target_distance_km') ? parseFloat(formData.get('target_distance_km')) : null,
                target_duration_hours: formData.get('target_duration_hours') ? parseFloat(formData.get('target_duration_hours')) : null,
            ...
            .then(res => {
                if (res.success) {
                    Swal.fire({
                        title: 'Success!',
                        text: res.message || 'Route saved successfully',
                        icon: 'success',
                        timer: 1500,
            ...
            .catch(err => {
                console.error(err);
                Swal.fire('Error', 'Failed to save route. Server error occurred.', 'error');
                saveBtn.disabled = false;
                saveBtn.innerText = originalText;
            });
        }
        ...
        function selectRoute(routeId) {
            ...
            document.getElementById('routeBuilderBtn').href = `/route-management/${routeId}/builder`;
