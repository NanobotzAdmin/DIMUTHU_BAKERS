...
@section('content')
                                ...
                                <tr class="bg-white hover:bg-gray-50 supervisor-row"
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <button id="live-loc-btn-sv-{{ $supervisor['id'] }}"
                                                data-supervisor-id="{{ $supervisor['id'] }}"
                                                onclick="fetchAndStoreLiveLocation('{{ $supervisor['id'] }}', '{{ $supervisor['superviser_name'] }}')"
                                                class="supervisor-live-loc-btn inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-400 px-2.5 py-1.5 rounded-lg border border-gray-200 transition-colors font-medium cursor-not-allowed" title="Checking status..." disabled>
            ...
            <div id="map-panel"
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div id="map-date-container" class="flex flex-wrap items-center gap-3">
                        <div class="flex items-center gap-1.5 relative" id="trackingTypeMultiSelectContainer">
                            <label class="text-xs font-medium text-gray-700">Type:</label>
                            <div class="relative">
                                <button type="button" id="trackingTypeDropdownBtn" onclick="toggleTrackingTypeDropdown()"
                                    class="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white flex items-center justify-between gap-2 min-w-[150px] shadow-sm hover:bg-gray-50 focus:ring-2 focus:ring-indigo-500">
                                    <span id="trackingTypeDropdownLabel" class="truncate text-xs font-medium text-gray-700">All Types Selected</span>
                                    <i class="bi bi-chevron-down text-xs text-gray-500"></i>
                                </button>
                                <!-- Dropdown Menu -->
                                <div id="trackingTypeDropdownMenu" class="hidden absolute left-0 mt-1 w-52 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-2 space-y-1">
                                    <label class="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer text-xs font-semibold text-indigo-700 border-b border-gray-100 pb-1.5">
                                        <input type="checkbox" id="typeSelectAll" checked onchange="toggleSelectAllTrackingTypes(this)" class="rounded text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5">
                                        Select All
                                    </label>
                                    <label class="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer text-xs text-gray-700">
                                        <input type="checkbox" value="1" checked onchange="handleTrackingTypeCheckboxChange()" class="tracking-type-cb rounded text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5">
                                        <span class="w-2 h-2 rounded-full bg-blue-500"></span> 30-Min Track
                                    </label>
                                    <label class="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer text-xs text-gray-700">
                                        <input type="checkbox" value="2" checked onchange="handleTrackingTypeCheckboxChange()" class="tracking-type-cb rounded text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5">
                                        <span class="w-2 h-2 rounded-full bg-emerald-500"></span> Start Trip
                                    </label>
                                    <label class="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer text-xs text-gray-700">
                                        <input type="checkbox" value="3" checked onchange="handleTrackingTypeCheckboxChange()" class="tracking-type-cb rounded text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5">
                                        <span class="w-2 h-2 rounded-full bg-purple-500"></span> Customer Billing
                                    </label>
                                    <label class="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer text-xs text-gray-700">
                                        <input type="checkbox" value="4" checked onchange="handleTrackingTypeCheckboxChange()" class="tracking-type-cb rounded text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5">
                                        <span class="w-2 h-2 rounded-full bg-amber-500"></span> Unload
                                    </label>
                                    <label class="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer text-xs text-gray-700">
                                        <input type="checkbox" value="5" checked onchange="handleTrackingTypeCheckboxChange()" class="tracking-type-cb rounded text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5">
    ...
    <script>
        // Fetch supervisor online status on load and refresh every 60 seconds
        let supervisorOnlineMap = {};

        (function() {
            fetchSupervisorOnlineStatus();
            setInterval(fetchSupervisorOnlineStatus, 60000);
        })();

        function fetchSupervisorOnlineStatus() {
            fetch('/api/supervisors/online-status')
                .then(res => res.json())
                .then(res => {
                    if (res.status && res.data) {
                        supervisorOnlineMap = {};
                        res.data.forEach(item => {
                            supervisorOnlineMap[item.supervisor_id] = item.is_online;
                        });
                        updateSupervisorLiveLocationButtons();
                    }
                })
                .catch(err => console.error('Failed to fetch supervisor online status:', err));
        }

        function updateSupervisorLiveLocationButtons() {
            document.querySelectorAll('.supervisor-live-loc-btn').forEach(btn => {
                const supervisorId = btn.getAttribute('data-supervisor-id');
                const label = btn.querySelector('.live-loc-label');
                const isOnline = supervisorOnlineMap[supervisorId] === true;

                if (isOnline) {
                    btn.disabled = false;
                    btn.className = 'supervisor-live-loc-btn inline-flex items-center gap-1 text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-2.5 py-1.5 rounded-lg border border-indigo-200 transition-colors font-medium cursor-pointer';
                    ...
                    btn.disabled = true;
                    btn.className = 'supervisor-live-loc-btn inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-400 px-2.5 py-1.5 rounded-lg border border-gray-200 transition-colors font-medium cursor-not-allowed';
                    btn.title = 'Supervisor is not logged into the mobile app';
                        ...
                        const infoWindow = new google.maps.InfoWindow({
                            content: `
                                <div class="p-2">
                                    ...
                                    ${getTrackingTypeBadge(loc.tracking_type)}
        ...
        // ==========================================
        // MULTI-SELECT TRACKING TYPE DROPDOWN LOGIC
        // ==========================================
        function toggleTrackingTypeDropdown() {
            const menu = document.getElementById('trackingTypeDropdownMenu');
            menu.classList.toggle('hidden');
        }

        // Close dropdown when clicking outside
        document.addEventListener('click', function(e) {
            const container = document.getElementById('trackingTypeMultiSelectContainer');
        ...
        function getSelectedTrackingTypes() {
            const cbs = document.querySelectorAll('.tracking-type-cb:checked');
            return Array.from(cbs).map(cb => cb.value);
        }

        function toggleSelectAllTrackingTypes(selectAllCb) {
            const cbs = document.querySelectorAll('.tracking-type-cb');
            cbs.forEach(cb => cb.checked = selectAllCb.checked);
            updateTrackingTypeDropdownLabel();
            handleMapDateChange();
        }

        function handleTrackingTypeCheckboxChange() {
            const cbs = document.querySelectorAll('.tracking-type-cb');
            const checkedCbs = document.querySelectorAll('.tracking-type-cb:checked');
            const selectAllCb = document.getElementById('typeSelectAll');
            if (selectAllCb) {
        ...
        function updateTrackingTypeDropdownLabel() {
            const checkedCbs = document.querySelectorAll('.tracking-type-cb:checked');
            const totalCbs = document.querySelectorAll('.tracking-type-cb');
            const label = document.getElementById('trackingTypeDropdownLabel');
            if (!label) return;
        ...
        function loadSupervisorHistoryFromDate() {
            ...
            const selectedTypes = getSelectedTrackingTypes();
            ...
            fetch(`/api/supervisors/${activeSupervisorId}/locations/history?date=${dateVal}`)
                ...
                .then(res => {
                    ...
                    history.forEach((point, index) => {
                        ...
                        const infoWindow = new google.maps.InfoWindow({
                            content: `<div class="p-2 text-xs font-semibold">${isStart ? 'Start: ' : (isEnd ? 'End: ' : 'Time: ')}${point.date}<br/>${getTrackingTypeBadge(point.tracking_type)}${point.description ? `<br/><span class="text-indigo-600 font-normal">${point.description}</span>` : ''}</div>`
                        });
        ...
        function getTrackingTypeBadge(type) {
            const types = {
        ...
        function fetchAndStoreLiveLocation(supervisorId, name) {
            // Check online status before proceeding
            if (supervisorOnlineMap[supervisorId] !== true) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Supervisor Offline',
            ...
            Swal.fire({
                title: 'Track Supervisor Live Location?',
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`/api/supervisors/${supervisorId}/store-live-location`, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(res => {
                        if (res.status) {
                            Swal.fire({
                                ...
                                text: `Supervisor live location recorded successfully!`,
                                timer: 1500,
                                showConfirmButton: false
                            });
                            openSupervisorHistoryMap(supervisorId, name);
                        } else if (res.offline) {
                            Swal.fire({
                                icon: 'warning',
                                title: 'Supervisor Offline',
                                text: res.message || 'Supervisor is not logged into the mobile app.',
                            });
